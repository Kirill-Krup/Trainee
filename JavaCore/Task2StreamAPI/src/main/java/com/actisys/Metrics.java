...
public class Metrics {
    ...
    public double getTotalIncomeForAllCompletedOrders(List<Order> allOrders){
        return allOrders.stream().filter(order -> order.getStatus() == OrderStatus.DELIVERED)
                .flatMap(order -> order.getItems().stream()).mapToDouble(item-> item.getPrice() * item.getQuantity()).sum();
    }

    public double getAverageCheckForDeliveredOrders(List<Order> allOrders){
        List<Order> completedOrders = allOrders.stream().filter(order -> order.getStatus() == OrderStatus.DELIVERED).collect(Collectors.toList());
    ...
    public List<Customer> getCustomersWithFivePlusOrders(List<Order> allOrders){
        List<Customer> fivePlusProductsCustomers = allOrders.stream().collect(Collectors.groupingBy(element ->
